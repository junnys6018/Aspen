...
The first stage in executing an Aspen program is called [lexing](https://en.wikipedia.org/wiki/Lexical_analysis). In this stage, the linear sequence of _characters_ in an Aspen program are converted into a linear sequence of _tokens_. Aspen's lexical grammar is [regular](https://en.wikipedia.org/wiki/Regular_grammar). The set of allowable tokens is specified in grammar below.
